      ...
      <div className="mx-auto grid max-w-[1440px] grid-cols-[auto_1fr_auto] items-center gap-6 px-6 py-4 md:px-10">
        ...
        <nav className="hidden justify-center gap-8 md:flex">
          {links.map(([label, href]) => (
            <Link key={label} href={href} className="link-draw text-[13.5px]" style={{ color: "var(--ink-soft)" }}>
              {label}
            </Link>
          ))}
        </nav>
  ...
  return (
    <div className="glass-card">
      ...
      <div className="mt-3 h-1.5 w-full overflow-hidden rounded-full" style={{ background: "rgba(21,23,28,0.08)" }}>
        <div className="progress-bar h-full rounded-full" style={{ background: "var(--ink)", "--target": "68%" } as React.CSSProperties} />
      </div>
      ...
      <div className="relative z-10 mx-auto grid max-w-[1440px] grid-cols-12 gap-6 px-6 pt-10 pb-24 md:px-10 md:pt-16 md:pb-32">
        ...
        <div className="col-span-12 mt-10 md:col-span-7 md:mt-14">
          ...
          >
            ...
            <Link href="/internship" className="btn-outline group">
...
function Courses() {
  ...
  return (
    <section className="mx-auto max-w-[1440px] px-6 py-24 md:px-10 md:py-28">
      ...
      <ul className="mt-4 divide-y" style={{ "--tw-divide-opacity": 1 } as React.CSSProperties}>
        {courses.map((c: { id: string; title: string; slug?: string; category?: string; modules?: unknown[]; duration?: string; level?: string; discountedPrice: number }, i: number) => (
          <Link href={`/courses/${c.slug || c.id}`} key={c.id || i} className="block group transition-all"
            style={{
      ...
      <div className="mt-10 flex justify-end">
        <Link href="/courses" className="link-draw text-[13px]" style={{ color: "var(--ink)" }}>
  ...
  const col = (h: string, items: [string, string][]) => (
    <div>
      ...
      <ul className="mt-4 space-y-2 text-[13px]">
        {items.map(([l, href]) => (
          <li key={l}>
            <Link href={href} className="link-draw" style={{ color: "var(--ink-soft)" }}>{l}</Link>
          </li>
        ))}
      </ul>
    </div>

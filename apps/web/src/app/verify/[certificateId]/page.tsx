...
const CERT_LABELS: Record<string, { title: string; icon: React.ElementType }> = {
...
export default function VerifyPage() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || "http://localhost:5000/api"}/internships/certificate/verify/${certificateId}`);
      return res.json();
    }
  });
          ...
          {isLoading ? (
            ...
            <div className="space-y-6">
              ...
              <div className="text-center">
                <Link href="/" className="text-[13px] font-medium" style={{ color: "var(--ink-mute)" }}>← Back to Enginow</Link>
              </div>
            </div>
          )}
        </div>

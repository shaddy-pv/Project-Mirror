...
export default function TrainingsPage() {
  const { data: trainings = [], isLoading } = useQuery(trainingsQueryOptions);
  const [search, setSearch] = useState("");

  const filtered = useMemo(() => {
    const list = Array.isArray(trainings) ? trainings : [];
    return (list as TrainingItem[]).filter((t) => {
      if (!t) return false;
      const query = search.toLowerCase();
      const title = (t.title || "").toLowerCase();
      const matchesTitle = title.includes(query);
      const skills = Array.isArray(t.youWillLearn) ? t.youWillLearn : [];
      const matchesSkills = skills.some((skill) => (skill || "").toLowerCase().includes(query));
                  ...
                  >
                    ...
                    >
                      ...
                      <div className="relative aspect-[16/9] flex items-center justify-center overflow-hidden" style={{ borderRadius: "23px 23px 0 0", background: "var(--amber-soft)", borderBottom: "0.8px solid rgba(21,23,28,0.06)" }}>
                        ...
                        {discountPercent > 0 && (
                          <div className="absolute right-3.5 top-3.5 rounded-full px-3 py-1 text-[10.5px] font-bold uppercase tracking-wider bg-moss text-paper shadow-sm">
                            {discountPercent}% OFF
                          </div>
                        )}
                      </div>
                      ...
                      <div className="flex flex-1 flex-col p-6">
                        ...
                        <div className="mt-5 mb-auto">
                          ...
                          <ul className="flex flex-wrap gap-2">
                            {(Array.isArray(t.youWillLearn) && t.youWillLearn.length > 0
                              ? t.youWillLearn
                              : ["Production Architecture", "Full-Stack Development", "Cloud & Deployment", "1-on-1 Mentorship"]
                            ).map((skill) => (
                              <li key={skill} className="inline-flex items-center gap-1.5 rounded-md px-2.5 py-1 text-[12px] font-medium" style={{ background: "rgba(21,23,28,0.05)", color: "var(--ink-soft)" }}>
                                <CheckCircle2 className="h-3 w-3 opacity-50 text-moss" />
                                {skill}
                              </li>
                            ))}
                          </ul>
                        ...
                        {/* Pricing & CTA */}
                        <div className="mt-6 flex items-center justify-between pt-5 gap-3" style={{ borderTop: "0.8px solid rgba(21,23,28,0.09)" }}>
                          <div className="min-w-0">
                            <p className="text-[11px] uppercase tracking-wider mb-0.5" style={{ color: "var(--ink-mute)" }}>Program Fee</p>
                            <div className="flex items-baseline gap-2">
                              <span className="text-xl font-bold leading-none" style={{ color: "var(--ink)" }}>₹{t.discountedPrice}</span>
                              <span className="text-[13px] line-through leading-none opacity-60" style={{ color: "var(--ink-mute)" }}>₹{t.originalPrice}</span>
                            </div>
                          </div>
                          <div className="flex items-center gap-1.5 shrink-0">
                            <button 
                              onClick={() => handleShare(t.id)}
                              className="inline-flex items-center gap-1.5 rounded-full px-2.5 py-2 text-[12px] font-medium transition-colors hover:bg-secondary/60"
                              style={{ color: "var(--ink-soft)" }}
                              title="Share"
                            >
                              <Share2 className="h-3.5 w-3.5" />
                              <span className="hidden sm:inline">{copiedId === t.id ? "Copied!" : "Share"}</span>
                            </button>
                            <Link 
                              href={`/trainings/${t.slug || t.id}`} 
                              className="btn-primary justify-center whitespace-nowrap shrink-0 px-4 py-2 text-[12.5px] font-semibold"
                            >

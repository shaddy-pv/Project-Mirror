import  createFileRoute, useNavigate, useSearch,  from 'next/link';
...
,
  head: () => ({
    meta: [
      { title: "Sign in — Enginow" },
      { name: "description", content: "Sign in or create an Enginow learner account." },
      { property: "og:title", content: "Sign in — Enginow" },
      { property: "og:description", content: "Sign in or create an Enginow learner account." },
...
export default function AuthPage() {
  ...
  const search = useSearch({ from: "/auth" }) as { redirect?: string; ref?: string };
  const destination = sanitizeRedirect(search.redirect) || "/";

  useEffect(() => {
    const ref = search.ref ?? localStorage.getItem("enginow_ref") ?? "";
    if (ref) {
      setReferralCode(ref);
          ...
          <form onSubmit={handleEmailAuth} className="space-y-4">
            {mode === "signup" && (
              <>
                ...
                <Field label={<>Referral Code <span className="normal-case font-normal opacity-60">(optional)</span></> as any}>

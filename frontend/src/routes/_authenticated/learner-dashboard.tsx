...
export const Route = createFileRoute("/_authenticated/learner-dashboard")({
  loader: ({ context }) => context.queryClient.ensureQueryData(enrollmentsQueryOptions),
  component: DashboardPage,
  head: () => ({
    meta: [
      { title: "Learner Dashboard — Enginow" },
      { name: "description", content: "Your Enginow learner dashboard." },
    ],
  }),
});
...
const CERT_TYPE_LABELS: Record<string, { label: string; icon: any }> = {
      ...
      <div className="space-y-0">
        ...
        <Section title="Internship Details">
          <div className="grid grid-cols-2 gap-3">
            <DetailRow label="Domain"   value={app.internship?.domain} />
            <DetailRow label="Location" value={app.internship?.locationType} />
            <DetailRow label="Duration" value={app.internship?.duration} />
            <DetailRow label="Type"     value={app.internship?.type} />
          </div>
        ...
        <div className="mx-auto max-w-[1440px]">
          <div className="flex flex-col gap-6 md:flex-row md:items-end md:justify-between">
            ...
            <div className="flex items-center gap-3 self-start">
              ...
              <Link to="/internship" className="btn-amber inline-flex">Internships <ArrowRight className="h-4 w-4" /></Link>
            </div>
          </div>
        ...
        <div className="mx-auto flex max-w-[1440px] gap-0 overflow-x-auto">
          {(["courses", "internships", "careers", "orders"] as const).map((tab) => (
            <button key={tab} onClick={() => setActiveTab(tab)} className="relative px-5 py-4 text-[13.5px] font-medium capitalize transition-colors whitespace-nowrap" style={{ color: activeTab === tab ? "var(--ink)" : "var(--ink-mute)" }}>
              {tab === "courses" ? `My Courses (${enrollments.length})` : tab === "internships" ? `My Internships (${apps.length})` : tab === "careers" ? `My Careers (${(careerApps as any[]).length})` : `My Orders (${(myOrders as any[]).length})`}
          ...
          {enrollments.length === 0 ? (
            <div className="mx-auto max-w-sm glass-shell text-center">
              <div className="glass-card" style={{ borderRadius: "23px" }}>
                ...
                <Link to="/courses" className="btn-primary mt-6 inline-flex justify-center">Browse courses <ArrowRight className="h-4 w-4" /></Link>
              </div>
            </div>
          ) : (
            <>
                  ...
                  return (
                    <motion.article key={enrollment.id} initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.28, delay: i * 0.06, ease: [0.16, 1, 0.3, 1] }} className="glass-shell group">
                      <div className="glass-card flex flex-col transition-all group-hover:-translate-y-1" style={{ borderRadius: "23px" }}>
                        ...
                        <Link to="/learn/$slug" params={{ slug: course.slug }} className="btn-primary mt-5 justify-center">
          ...
          {apps.length === 0 ? (
            <div className="mx-auto max-w-sm glass-shell text-center">
              <div className="glass-card" style={{ borderRadius: "23px" }}>
                ...
                <Link to="/internship" className="btn-primary mt-6 inline-flex justify-center">Browse internships <ArrowRight className="h-4 w-4" /></Link>
              </div>
            </div>
          ) : (
            <div>
          ...
          {careerApps.length === 0 ? (
            <div className="mx-auto max-w-sm glass-shell text-center">
              <div className="glass-card" style={{ borderRadius: "23px" }}>
                ...
                <Link to="/careers" className="btn-primary mt-6 inline-flex justify-center">Browse careers <ArrowRight className="h-4 w-4" /></Link>
              </div>
            </div>
          ) : (
            <div>
              ...
              <div className="space-y-3">
                {careerApps.map((app: any, i: number) => {
          ...
          {(myOrders as any[]).length === 0 ? (
            <div className="mx-auto max-w-sm glass-shell text-center">
              <div className="glass-card" style={{ borderRadius: "23px" }}>
                ...
                <a href="/shop" className="btn-primary mt-6 inline-flex justify-center">Visit Shop <ArrowRight className="h-4 w-4" /></a>
              </div>
            </div>
          ) : (
            <div>
              ...
              <div className="space-y-4">
                {(myOrders as any[]).map((order: any, i: number) => {
                    ...
                    >
                      ...
                      <div className="flex items-center gap-4 px-5 py-4">
                        ...
                        <div className="text-right shrink-0">
                          <p className="font-bold text-[16px]" style={{ color: "var(--ink)" }}>₹{order.amount}</p>
                      ...
                      {order.address && (
                        <div className="px-5 pb-3 border-t hairline pt-3" style={{ borderColor: "rgba(21,23,28,0.06)", background: "rgba(21,23,28,0.02)" }}>
                          ...
                          <p className="text-[12px]" style={{ color: "var(--ink-soft)" }}>{order.address.city}, {order.address.state} — {order.address.pincode}</p>
                        </div>
                      )}
...
function OrderRateBlock({ order }: { order: any }) {
  ...
  const handleRate = async (val: number) => {
    ...
    try {
      ...
      setRating(val);
      order.rating = val; // update local ref
    } catch (err: any) {
      alert(err.message);
    } finally {

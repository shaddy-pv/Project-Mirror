...
export const Route = createFileRoute("/trainings/$slug")({
  ...
  loader: ({ params, context }) =>
    context.queryClient.ensureQueryData(trainingQueryOptions(params.slug)),
  component: TrainingDetailPage,
  head: ({ params }) => ({
    meta: [
      { title: `${params.slug} — Enginow` },
      { name: "description", content: "Course details on Enginow." },
      { property: "og:title", content: `${params.slug} — Enginow` },
      { property: "og:description", content: "Course details on Enginow." },
...
function TrainingDetailPage() {
  ...
  async function handleEnroll() {
    if (!isAuthenticated) {
      navigate({ to: `/auth?redirect=/trainings/${training.slug}` as any });
      return;
    }
    setEnrolling(true);
    try {
      ...
      if (finalPrice > 0) {
        ...
        await new Promise<void>((resolve, reject) => {
          const rzp = new window.Razorpay({
            key: import.meta.env.VITE_RAZORPAY_KEY_ID, amount: finalPrice * 100, currency: "INR",
      ...
      navigate({ to: `/learn/${training.slug}` as any });
    } catch (err: any) {
      if (err?.message !== "dismissed") { console.error(err); alert("Something went wrong. Please try again."); }
    ...
    <main className="relative min-h-screen" style={{ background: "#FFFFFF" }}>
      ...
      <section className="relative px-6 pb-16 pt-24 md:px-10" style={{ borderBottom: "0.8px solid rgba(21,23,28,0.09)" }}>
        <div className="mx-auto max-w-[1440px]">
          <Link to="/courses" className="inline-flex items-center gap-1.5 text-[13px] transition-colors" style={{ color: "var(--ink-mute)" }}>
          ...
          <div className="grid gap-10 lg:grid-cols-[1fr_340px]">
            ...
            <aside className="h-fit glass-shell">
              <div className="glass-card" style={{ borderRadius: "23px" }}>
                ...
                {isEnrolled && (
                  <Link to="/learn/$slug" params={{ slug: training.slug }} className="btn-amber mt-6 w-full justify-center">

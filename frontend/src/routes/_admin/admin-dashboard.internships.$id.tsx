...
import { adminGetInternship, adminUpdateInternship } from "@/lib/admin.functions";

export const Route = createFileRoute("/_admin/admin-dashboard/internships/$id")({
  component: EditInternshipPage,
});
...
function EditInternshipPage() {
  ...
  useEffect(() => {
    adminGetInternship(id).then(data => {
      setForm({
        title: data.title || "",
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setError(null);
    try {
      await adminUpdateInternship({ data: { id, ...form } });
  ...
  if (fetching) {
    return <div className="px-8 py-10 flex items-center gap-2 text-ink-mute"><Loader2 className="animate-spin h-4 w-4"/> Loading...</div>;
  }
  ...
  return (
    <div className="px-8 py-10">
      ...
      <h1 className="mt-4 text-2xl font-semibold tracking-tight">Edit Internship</h1>

      {error && <div className="mt-4 rounded-md bg-red-50 p-4 text-[13px] text-red-600">{error}</div>}

...
import { Loader2, ArrowLeft, Plus, X, ChevronDown, ChevronUp, Play, FileText, IndianRupee, Upload, Link as LinkIcon } from "lucide-react";
import { adminUpdateTraining, adminGetTraining } from "@/lib/admin.functions";

export const Route = createFileRoute("/_admin/admin-dashboard/trainings/$id")({
  loader: async ({ params }) => {
    return adminGetTraining(params.id);
  },
...
function EditTrainingPage() {
  ...
  const training = Route.useLoaderData() as any;
  ...
  const [form, setForm] = useState({
    ...
    discountedPrice: training.discountedPrice || 0,
    youWillLearn: (training.youWillLearn || []).join(", "),
    bannerUrl: training.bannerUrl || "",
    status: training.status || "draft",
    roadmap: [] as CourseModule[],
  });

  const [modules, setModules] = useState<CourseModule[]>(training.roadmap?.length ? training.roadmap : [emptyModule()]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const youWillLearnArray = form.youWillLearn
        .split(",")
        .map((s: string) => s.trim())
        .filter(Boolean);
        
      await adminUpdateTraining({ 
        data: {
          id: training.id,
          ...form,
          roadmap: modules,
          youWillLearn: youWillLearnArray
        }
      });
      navigate({ to: "/admin-dashboard/trainings" });
    } catch (err: any) {
  ...
  return (
    <div className="px-8 py-10">
      <button
        ...
        className="mb-6 inline-flex items-center gap-2 text-[13px] text-ink-soft hover:text-ink transition-colors"
      >
      ...
      <div className="flex items-end justify-between">
        <div>
          ...
          <p className="mt-1 text-[13.5px] text-ink-soft">Modify training details and roadmap.</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="mt-8 max-w-2xl space-y-7">
        ...
        <div className="space-y-4 rounded-xl border hairline bg-card p-6">
          ...
          <div className="grid gap-6 md:grid-cols-2">
            <div>
              <label className="mb-1.5 block text-[12px] font-semibold tracking-wider text-ink-mute uppercase">
                Original Price (₹)
              </label>
              <input
                type="number"
                ...
                className="w-full rounded-md border hairline bg-transparent px-3 py-2 text-[13.5px] outline-none focus:border-ink"
              />
            </div>
          ...
          <div>
            <label className="mb-1.5 block text-[12px] font-semibold tracking-wider text-ink-mute uppercase">
              You Will Learn (comma separated)
            </label>
            <input
              value={form.youWillLearn}
              onChange={(e) => setForm({ ...form, youWillLearn: e.target.value })}
              placeholder="React, Next.js, System Design..."
              className="w-full rounded-md border hairline bg-transparent px-3 py-2 text-[13.5px] outline-none focus:border-ink"
            />
          </div>
        </div>

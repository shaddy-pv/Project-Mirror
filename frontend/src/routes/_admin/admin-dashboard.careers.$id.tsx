...
import { adminGetCareer, adminUpdateCareer } from "@/lib/admin.functions";

export const Route = createFileRoute("/_admin/admin-dashboard/careers/$id")({
  loader: ({ params }) => adminGetCareer(params.id),
...
function EditCareerPage() {
  ...
  const [form, setForm] = useState({
    title: initial.title || "",
    domain: initial.domain || "Software Engineering",
    description: initial.description || "",
    locationType: initial.locationType || "Remote",
    tags: initial.tags || "",
    responsibilities: initial.responsibilities || "",
    perks: initial.perks || [],
    openFrom: initial.openFrom ? new Date(initial.openFrom).toISOString().split("T")[0] : new Date().toISOString().split("T")[0],
    openUntil: initial.openUntil ? new Date(initial.openUntil).toISOString().split("T")[0] : new Date(Date.now() + 30 * 24 * 60 * 60 * 1000).toISOString().split("T")[0],
  });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setError(null);
    try {
      await adminUpdateCareer({ data: { id: initial.id, ...form } });
  ...
  return (
    <div className="px-8 py-10">
      ...
      <h1 className="mt-4 text-2xl font-semibold tracking-tight">Edit Career</h1>
                ...
                <input 
                  ...
                  onChange={(e) => {
                    const newPerks = e.target.checked 
                      ? [...form.perks, perk] 
                      : form.perks.filter((p: string) => p !== perk);
                    set("perks", newPerks);
                  }}

...
import { ArrowLeft, Search, MapPin, Briefcase, ChevronRight, CheckCircle2, Loader2, X } from "lucide-react";
...
export const Route = createFileRoute("/careers")({
  loader: ({ context }) => context.queryClient.ensureQueryData(careersQueryOptions),
  component: CareersPage,
  head: () => ({
    meta: [
      { title: "Careers — Enginow" },
      { name: "description", content: "Apply for full-time job opportunities at Enginow." },
    ],
  }),
});
...
function CareersPage() {
  ...
  const appliedIds = useMemo(() => new Set((myApps as any[]).map(a => a.careerId)), [myApps]);
...
function ApplyModal({ career, onClose }: { career: CareerItem | null; onClose: () => void }) {
  ...
  return (
    <Dialog.Root open={!!career} onOpenChange={(open) => !open && onClose()}>
      <AnimatePresence>
        {career && (

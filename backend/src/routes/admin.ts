...
import { courses, userRoles, profiles, enrollments, trainings, products, orders } from "../collections";
...
router.post("/internship-applications/:id/certificate", requireFirebaseAuth, async (req: AuthenticatedRequest, res) => {
  try {
    ...
    if (app.status !== "accepted") return res.status(400).json({ error: "Can only issue certificates for accepted applications" });

    const { type, customDocumentBase64 } = req.body; // "completion" | "lor" | "loe"
    ...
    const cert = {
      ...
      customDocumentBase64, // the uploaded file as a base64 string
    };
